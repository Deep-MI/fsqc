...
def readAparcStats(path_aparc_stats, hemi):
    ...
    if col_headers is not None:
        for line in aparc_stats:
            if 'bankssts' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'bankssts' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'caudalanteriorcingulate' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'caudalanteriorcingulate' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'caudalmiddlefrontal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'caudalmiddlefrontal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'cuneus' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'cuneus' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'entorhinal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'entorhinal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'fusiform' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'fusiform' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'inferiorparietal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'inferiorparietal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'inferiortemporal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'inferiortemporal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'isthmuscingulate' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'isthmuscingulate' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'lateraloccipital' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'lateraloccipital' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'lateralorbitofrontal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'lateralorbitofrontal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'lingual' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'lingual' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'medialorbitofrontal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'medialorbitofrontal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'middletemporal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'middletemporal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'parahippocampal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'parahippocampal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'paracentral' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'paracentral' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'parsopercularis' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'parsopercularis' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'parsorbitalis' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'parsorbitalis' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'parstriangularis' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'parstriangularis' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'pericalcarine' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'pericalcarine' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'postcentral' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'postcentral' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'posteriorcingulate' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'posteriorcingulate' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'precentral' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'precentral' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'precuneus' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'precuneus' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'rostralanteriorcingulate' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'rostralanteriorcingulate' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'rostralmiddlefrontal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'rostralmiddlefrontal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'superiorfrontal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'superiorfrontal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'superiorparietal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'superiorparietal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'superiortemporal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'superiortemporal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'supramarginal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'supramarginal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'frontalpole' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'frontalpole' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'temporalpole' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'temporalpole' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'transversetemporal' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'transversetemporal' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
            elif 'insula' in line:
                aparc.update({"aparc" + "." + hemi + '.' + 'insula' : dict(zip(col_headers[1:], [float(x) for x in line.split()[1:]]))})
...
def outlierDetection(subjects, subjects_dir, output_dir, outlierDict, min_no_subjects=10, hypothalamus=False, hippocampus=False, hippocampus_label=None):
    """
    ...
    for subject in subjects:
        ...
        if hippocampus is True and hippocampus_label is not None:
            ...
            path_hippocampus_stats = os.path.join(subjects_dir, subject, "mri", "lh.hippoSfVolumes-"+hippocampus_label+".txt")
            ...
            path_hippocampus_stats = os.path.join(subjects_dir, subject, "mri", "rh.hippoSfVolumes-"+hippocampus_label+".txt")
            if os.path.exists(path_hippocampus_stats):
                hippocampus_stats = readHippocampusStats(path_hippocampus_stats, hemi="rh", prefix="hippocampus")
                regions[subject].update(hippocampus_stats)
                all_regions_keys.extend(list(hippocampus_stats.keys()))

            path_hippocampus_stats = os.path.join(subjects_dir, subject, "mri", "lh.amygNucVolumes-"+hippocampus_label+".txt")
            if os.path.exists(path_hippocampus_stats):
                hippocampus_stats = readHippocampusStats(path_hippocampus_stats, hemi="lh", prefix="amygdala")
                regions[subject].update(hippocampus_stats)
                all_regions_keys.extend(list(hippocampus_stats.keys()))

            path_hippocampus_stats = os.path.join(subjects_dir, subject, "mri", "rh.amygNucVolumes-"+hippocampus_label+".txt")
            if os.path.exists(path_hippocampus_stats):
    ...
    if len(subjects) >= min_no_subjects:
        ...
        for subject in regions:
            ...
            for key in regions[subject]:
                ...
                if (regions[subject][key] < sample_nonpar_lower[key]) or (regions[subject][key] > sample_nonpar_upper[key]):
                    nonparDict.update({key : True })
                else:
                    nonparDict.update({key : False })

                if (regions[subject][key] < sample_param_lower[key]) or (regions[subject][key] > sample_param_upper[key]):
    ...
    for subject in regions:
        ...
        for key in regions[subject]:
            ...
            if outlierKey in outlierDict:
                ...
                if (regions[subject][key] < outlierDict[outlierKey]['lower']) or (regions[subject][key] > outlierDict[outlierKey]['upper']):
    ...
    with open(os.path.join(output_dir, 'all.regions.stats'), 'w') as datafile:
        csvwriter = csv.DictWriter(datafile, fieldnames=regionsFieldnames, delimiter=',',quotechar='"', quoting=csv.QUOTE_MINIMAL)
