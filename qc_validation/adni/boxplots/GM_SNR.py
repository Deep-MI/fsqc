...
#Get the gm data: 
gm = data_fs.SNR_gray_matter_norm.tolist()
gm.pop(0)
gm = [float(i) for i in gm]
...
for subject in ID_80_match: 
    nb_holes_80.append(gm[sbj_fs.index(subject)])
...
for subject in ID_70_match: 
    nb_holes_70.append(gm[sbj_fs.index(subject)])
...
for subject in ID_60_match: 
    nb_holes_60.append(gm[sbj_fs.index(subject)])
...
for subject in ID_50_match: 
    nb_holes_50.append(gm[sbj_fs.index(subject)])
...
layout = go.Layout(
    title='Gray Matter SNR over age ',
    yaxis=dict(
        autorange=True,
        showgrid=True,
        zeroline=True,
