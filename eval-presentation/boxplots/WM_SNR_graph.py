# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 16:16:34 2019
...
#Get the wm data: 
wm = data_fs.SNR_white_matter_norm.tolist()
wm.pop(0)
wm = [float(i) for i in wm]
...
for subject in ID_80_match: 
    nb_holes_80.append(wm[sbj_fs.index(subject)])
...
for subject in ID_70_match: 
    nb_holes_70.append(wm[sbj_fs.index(subject)])
...
for subject in ID_60_match: 
    nb_holes_60.append(wm[sbj_fs.index(subject)])
...
for subject in ID_50_match: 
    nb_holes_50.append(wm[sbj_fs.index(subject)])
...
layout = go.Layout(
    title='White Matter SNR over age ',
    yaxis=dict(
        autorange=True,
        showgrid=True,
        zeroline=True,
