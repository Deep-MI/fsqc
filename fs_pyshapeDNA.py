...
# TODO
# - consider dependency on gmsh and meshfix to allow for tetra processing
# - removed some options because the binary is more powerful than the python
#   module
...
If "meshfix" and "gmsh" are in the path, it can also create and process
...
def which(program):
    """
    check if a program is executable, and return it with its path
    """
    def is_exe(fpath):
...
# Create a tetrahedron mesh by calling meshfix and gmsh
...
def run_shapeDNAtetra(tetmesh,outev,options):
    ...
    if options.bcond == 0:
        dirichlet = " --dirichlet"
    cmd =sdna+' --mesh '+tetmesh+' --num '+str(options.num)+' --outfile '+outev+dirichlet
    if options.evec:
        cmd = cmd+' --evec'
    run_cmd(cmd,'shapeDNA-tetra failed?') 


# Function to run shapeDNAtria
def run_shapeDNAtria(surf,outev,outsurf,options):
 # options : num, evec, refmin, tsmooth, gsmooth
    sdnahome = os.getenv('SHAPEDNA_HOME')
    ...
    if options.bcond == 0:
        dirichlet = " --dirichlet"
    cmd =sdna+' --mesh '+surf+' --num '+str(options.num)+' --outfile '+outev+dirichlet
    if options.evec:
        cmd = cmd+' --evec'
    if options.refmin > 0:
