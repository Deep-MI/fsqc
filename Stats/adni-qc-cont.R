...
# read QC data
q1<-read.csv("/groups/ag-reuter/projects/adni-qc/QC_1_5.csv")
q2<-read.csv("/groups/ag-reuter/projects/adni-qc/quality_checker_v2.csv")
q<-rbind(q1,q2)

# merge with quality checker table
m5<-m
m<-merge(m,q,by.x="Image.ID",by.y="Subject")
m<-droplevels(m)
