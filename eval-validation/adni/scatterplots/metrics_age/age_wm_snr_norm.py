...
for subject in sbj_match_full: 
    ...
    wm_snr_graph.append(wm_snr[index_fs])
    
    index_full = sbj_full.index(subject)
    age_graph.append(age_full[index_full])

slope, intercept, r_value, p_value, std_err = stats.linregress(age_graph,wm_snr_graph)
...
layout= go.Layout(
    title= 'Gray matter signal to noise ratio over age based on the norm image',
    hovermode= 'closest',
    xaxis= dict(
        title= 'Age',
        dtick=2,
    ...
    yaxis=dict(
        title= 'Gray matter signal to noise ratio',
        dtick=0.25,
        ticklen= 5,
        gridwidth= 2,
    ),
...
plotly.offline.plot(fig, filename = 'age-gm-snr-norm-scatter.html')
