...
#List of cnr alzheimer patients 
cnr_AD_graph = [] 
age_AD_graph = []
for subject in sbj_AD: 
    index_fs = sbj_fs.index(subject)
    cnr_AD_graph.append(cnr[index_fs])
    
    index_full = sbj_full.index(subject)
    age_AD_graph.append(age_full[index_full])

#List of CNR control subjects
cnr_CN_graph = []
age_CN_graph = [] 
for subject in sbj_CN: 
    index_fs = sbj_fs.index(subject)
    cnr_CN_graph.append(cnr[index_fs])
    
    index_full = sbj_full.index(subject)
    age_CN_graph.append(age_full[index_full])

#List of CNR beginning Alzheimer
cnr_MCI_graph = []
age_MCI_graph = [] 
for subject in sbj_MCI: 
    index_fs = sbj_fs.index(subject)
    cnr_MCI_graph.append(cnr[index_fs])
    
    index_full = sbj_full.index(subject)
    age_MCI_graph.append(age_full[index_full])

#Generate Linear fit for alzheimer disease patients
slope_AD, intercept_AD, r_value_AD, p_value_AD, std_err_AD = stats.linregress(age_AD_graph,cnr_AD_graph)
line_AD = slope_AD*np.asarray(age_AD_graph)+intercept_AD

#Generate linear fit for control patients
slope_CN, intercept_CN, r_value_CN, p_value_CN, std_err_CN = stats.linregress(age_CN_graph,cnr_CN_graph)
line_CN = slope_CN*np.asarray(age_CN_graph)+intercept_CN

#Generate linear fit for control patients
slope_MCI, intercept_MCI, r_value_MCI, p_value_MCI, std_err_MCI = stats.linregress(age_MCI_graph,cnr_MCI_graph)
...
trace0 = go.Scatter(
    ...
    name = 'CNR for Alzheimer disease patients',
...
trace4 = go.Scatter(
    ...
    name='CNR for the slight Alzheimer disease subjects'
)
trace5 = go.Scatter(
    x=age_MCI_graph,
    y=line_MCI,
...
layout= go.Layout(
    title= 'Contrast to noise ratio over age and illness',
    hovermode= 'closest',
    xaxis= dict(
        title= 'Age',
        dtick=2,
...
plotly.offline.plot(fig, filename = 'age-illness-cnr-scatter.html')
