...
- Interface changes: importing and running the scripts as a python package has changed. See [here](https://github.com/Deep-MI/fsqc#usage) for details. In particular, `run_fsqc` is now an executable file and replaces the `qatools` or `qatools.py` command.
